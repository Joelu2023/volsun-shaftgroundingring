...
const path = "/knowledge-center/technical-articles";
...
export default async function KnowledgeCenterTechnicalArticlesPage({ params }: Props) {
  const { locale: raw } = await params;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <ArticleListSection locale={locale} items={technicalArticles} emptyMessage={m.description} />
      <p className="mt-8 text-sm text-slate-600">
        <Link href={`/${locale}/knowledge-center/news`} className="text-brand-orange hover:underline">
          {locale === "zh" ? "浏览新闻动态" : "Browse news"}
        </Link>
      </p>
      <div className="mt-12">
